...
def parse_log(d):
    ...
    for line in p.read_text(errors="ignore").replace("\r", "\n").splitlines():
        ...
        update_s = float(m.group("update_s"))
        step_s = data_s + update_s
        rows.append(
            {
                "timestamp": m.group("ts"),
                "exact_step": step,
                "measurement": step > WARMUP_STEPS,
    ...
    with p.open(newline="", errors="ignore") as f:
        ...
        for raw in reader:
            ...
            rows.append(
                {
                    "timestamp": r.get(tk, ""),
                    "gpu_util": fnum(r.get(uk)) if uk else None,
...
def main():
    ...
    for nw in WORKERS:
        ...
        if (
            status == "failed"
            and (d / "train.log").exists()
            and "out of memory" in (d / "train.log").read_text(errors="ignore").lower()
        ):
            status = "oom"
        ...
        wall_sps = (
            (BATCH_SIZE * TOTAL_STEPS / wall)
            if status == "completed" and isinstance(wall, (int, float)) and wall > 0
            else None
        )
        out.append(
            {
                ...
                "measurement_steps": TOTAL_STEPS - WARMUP_STEPS,
                "metric_points": len(metrics),
                "measurement_points": len(meas),
                "wall_seconds": wall if wall is not None else "",
                "wall_samples_s": wall_sps if wall_sps is not None else "",
                ...
                "estimated_epoch_time_min": (mean_step * STEPS_PER_EPOCH / 60)
                if mean_step is not None
                else "",
                "mean_data_s": mean(r["data_s"] for r in meas) or "",
                "mean_update_s": mean(r["update_s"] for r in meas) or "",
                "log_samples_s": log_sps if log_sps is not None else "",
                "final_loss": metrics[-1]["loss"] if metrics else "",
                "final_grad_norm": metrics[-1]["grad_norm"] if metrics else "",
                "mean_gpu_util": mean(r["gpu_util"] for r in gpu) or "",
                "p95_gpu_util": pct([r["gpu_util"] for r in gpu], 0.95) or "",
                "peak_memory_mib": max(
                    [r["memory_used_mib"] for r in gpu if r["memory_used_mib"] is not None], default=""
                ),
                "mean_power_w": mean(r["power_w"] for r in gpu) or "",
                "gpu_samples": len(gpu),
            }
        )
