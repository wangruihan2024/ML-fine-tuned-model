...
def main():
    ...
    with profile(
        ...
        for step in range(1, TOTAL_STEPS + 1):
            ...
            with record_function("Forward"), accelerator.autocast():
            ...
            rows.append(
                {
                    ...
                    "gpu_memory_mib": float(torch.cuda.max_memory_allocated() / 1024 / 1024)
                    if torch.cuda.is_available()
                    else math.nan,
                }
            )
            ...
            if step % 10 == 0:
                print(
                    f"[profile] step={step}/{TOTAL_STEPS} loss={rows[-1]['loss']:.4f} total_s={rows[-1]['total_s']:.3f}",
                    flush=True,
                )
    ...
    try:
        table = prof.key_averages().table(
            sort_by="cuda_time_total" if torch.cuda.is_available() else "cpu_time_total", row_limit=60
        )
    ...
    meta = {
        ...
        "max_memory_allocated_mib": float(torch.cuda.max_memory_allocated() / 1024 / 1024)
        if torch.cuda.is_available()
        else None,
    }
